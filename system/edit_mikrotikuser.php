				...
				if(!empty($_REQUEST['username'])){
					...
					if($_REQUEST['username']==$_GET['id']){
	                   ...
	                   mysql_query("UPDATE mt_gen SET user='".$_REQUEST['username']."', pass='".$_REQUEST['password']."', limit_uptime='".$_REQUEST['limit_uptime']."', profile='".$_REQUEST['profile']."', ip_address='".$_REQUEST['ip']."', mac_address='".$_REQUEST['mac']."', comment='".$db_comment."', email='".$_REQUEST['email']."' WHERE user='".$user."'");
						...
						echo "<script language='javascript'>swal('บันทึกสำเร็จ!','แก้ไข  ".$username." สำเร็จแล้ว','success').then(function () {
            ...
            <div class="<?php print panel_modify();?>">
               ...
               echo button_btn_submit_account($account,$text_success,$bottonbtn_success);
				?>
                         ...
                         <button id="btnCancel" class="btn btn-danger" type="reset"  Onclick="window.location.href = 'index.php?page=mikrotikuser'">&nbsp;&nbsp;&nbsp;<i class="fa fa-times"></i>&nbsp;ยกเลิก&nbsp;&nbsp;&nbsp;</button>&nbsp;&nbsp;
						 <span class="hidden-xs"><button id="btnSave" class="btn btn-warning" type="reset">&nbsp;&nbsp;&nbsp;<i class="fa fa-refresh"></i>&nbsp;รีเซ็ต&nbsp;&nbsp;&nbsp;</button>&nbsp;&nbsp;<button type="button" class="btn btn-info" data-toggle="collapse" data-target="#manual">ข้อแนะนำการใช้งาน </button></span>
                        </div>
						</div>
						</form>
						 </div>
