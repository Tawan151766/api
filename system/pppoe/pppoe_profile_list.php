<?php
  ...
  if (dismiss === 'overlay') {
	...
	#######################################################
			...
			$num_check=0;
				for($i=0;$i < count($_REQUEST['check']);$i++){
					...
					#######ADD################
					$sql1="SELECT * FROM pppoe_pro WHERE pro_name='".$pro_add."'";
		$query1=mysql_query($sql1);
		$rows1=mysql_num_rows($query1);
		
		if($rows1 >0){
			
		$query2=mysql_query("SELECT * FROM pppoe_pro WHERE mt_id='".$id."'");
		 ...
		 }if(Empty($result['pro_name'])){ echo "<script language='javascript'>swal('Error Profile name Try again!','มีชื่อ นี้แล้วใน database กรุณาเลือกใหม่!','error').then(function () {window.location.href='index.php?page=pppoe_profile_list';}, function (dismiss) {if (dismiss === 'overlay') {window.location.href = 'index.php?page=pppoe_profile_list';}})</script>";}	
		}else{	
		########ADD########
					$num=count($_REQUEST['check']);
	...
	echo "<script language='javascript'>swal({
				...
				}else{echo "<script language='javascript'>swal('Error to Count Try again!','เกิดผิดพลาดในการนับจำนวน กรุณาลองใหม่!','error').then(function () {window.location.href='index.php?page=pppoe_profile_list&cancel=yes';}, function (dismiss) {if (dismiss === 'overlay') {window.location.href = 'index.php?page=pppoe_profile_list&cancel=yes';}})</script>";}	
                     ...
                     <div class="<?php print panel_modify();?>">
                        ...
                        <table class="table table-striped table-hover" id="dataTables-example">
                            ...
                            <div class="form-group input-group">                                        
									   ...
									   <?php
									    $bottonbtn_danger="on";
				$text_danger="<i class=\"fa fa-times\"></i>&nbsp;ลบ&nbsp;";
