        ...
        <form name="name" action="" method="post">
            <div class="<?php print panel_modify();?>">
                <div class="<?php print $panel_heading;?>"><i class="fa fa-user"></i>
                    <strong>User&nbsp;Usermanager</strong>
                </div>
                    ...
                    <table class="table table-striped table-hover" id="dataTables-example">
                        <thead>
                            <tr>
                                ...
                                <th>Mac Address</th>
                                <th>โปรไฟล์</th>
                                <th>อัพโหลด/ดาวน์โหลด</th>
                                <!-- <th>START DATE/TIME</th> -->
                                <th>คอมเม้นหมดอายุ</th>
                        ...
                        <?php
							...
							echo "<a class=\"btn btn-danger btn-xs\"\"><span></span> หมดอายุ </a>";
						...
						echo "<td class=\"text-right\">";
							...
							if($connect!=0){ echo "<a class=\"btn btn-success btn-xs\" title= \"click to kick user online\" onclick=\"swal({
								title: 'คุณแน่ใจไหม?',
								text: 'ต้องการจะKick  ".$ARRAY[$i]['username']."  ใช่หรือไม่',
								type: 'warning',
								...
								echo "<a class=\"btn btn-black btn-xs\" title= \"click to disable\" href=\"index.php?page=disable&return=userman&user=".$ARRAY[$i]['username']."\"><span></span> ปิด </a>&nbsp;&nbsp;";}else{
								
								echo "<a class=\"btn btn-black btn-xs\" title= \"click to disable\" onclick=\"
									swal('Can Not Disable User  ".$ARRAY[$i]['username']."!','จะมีผลกับการสร้างuser ในprofile ".$ARRAY[$i]['username']."','error').then(function () {
									window.location.href = 'index.php?page=usermanager';}, function (dismiss) {if (dismiss === 'overlay') {
									window.location.href = 'index.php?page=usermanager';}})\";><span></span> ปิด </a>&nbsp;&nbsp;";}
									
									
									}else{
									
									echo "<a class=\"btn btn-black btn-xs\" title= \"ปิดการใช้งาน\"href=\"index.php?page=enable&return=userman&user=".$ARRAY[$i]['username']."\"><span></span> Disable </a>&nbsp;&nbsp;";
									}
									#########################EDIT####################################
									$result=mysql_fetch_array(mysql_query("SELECT * FROM mt_gen WHERE user='".$ARRAY[$i]['username']."'"));
									...
									echo"<a class=\"btn btn-danger btn-xs\" title= \"click to remove\"
										onclick=\"swal({
										title: 'คุณแน่ใจไหม?',
										text: 'ต้องการจะลบ  ".$ARRAY[$i]['username']."  ใช่หรือไม่',
										type: 'warning',
										//allowOutsideClick: false,
										//showCloseButton: true,
										showCancelButton: true,
										...
										}else{ echo "<a class=\"btn btn-success btn-xs\" title= \"click to kick user online\" ><span class=\"fa fa-wifi\">  </span> ".$ARRAY[$i]['active-sessions']." </a>&nbsp;&nbsp;";
										}
										if($ARRAY[$i]['disabled']=="false"){
										echo "<a class=\"btn btn-black btn-xs\" title= \"click to disable\" ><span></span> ปิด </a>&nbsp;&nbsp;";
										}else{
										echo "<a class=\"btn btn-black btn-xs\" title= \"ปิดการใช้งาน\"><span></span> Disable </a>&nbsp;&nbsp;";
										}
