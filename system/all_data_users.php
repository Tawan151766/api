...
#######################################################################
			if($_REQUEST['active']=="remove"){
				...
				echo "<script language='javascript'>swal('ลบสำเร็จแล้ว!','ลบผู้ใช้งานจำนวน ".$num."  Users สำเร็จแล้ว','success').then(function () {
...
for($i=0; $i<$num3; $i++){if($ARRAY3[$i]['username']==$db_user){$db_user="";$new_man_fail=$new_man_fail+($mik_check+1);}}
	...
	$mik_newadd2=$mik_newadd2+($mik_check+1);
	 $ARRAY = $API->comm("/ip/hotspot/user/add", array(
			 ...
			 echo "<script language='javascript'>swal('Error transfer from ".$num." user!','hotspot สำเร็จ ".($mik_newadd1+$mik_newadd2)." กรุณาตรวจสอบ!','info').then(function () {
    window.location.href = 'index.php?page=all_data_users&id=".$id."';}, function (dismiss) {
  if (dismiss === 'overlay') {
    window.location.href = 'index.php?page=all_data_users&id=".$id."';
   }})</script>";
		exit();}else{echo "<script language='javascript'>swal('transfer Successfully','เพิ่ม user สำเร็จแล้ว จำนวนทั้งหมด ".($mik_newadd1+$mik_newadd2)." users','success').then(function () {
    window.location.href ='index.php?page=all_data_users&id=".$id."';}, function (dismiss) {
  if (dismiss === 'overlay') {
    window.location.href ='index.php?page=all_data_users&id=".$id."';
   }})</script>";
                ...
                <div class="col-lg-12">
                    <div class="<?php print panel_modify();?>">
                        <div class="<?php print $panel_heading;?>">
                            ...
                            <strong>ผู้ใช้งานทั้งหมด</strong>
                        </div>
                        <div class="panel-body">
						   ...
						   $check_comment=substr("".$result['comment']."",-30,20);
					...
					######## }} ######จบสคริปคัดกรอง comment ###
						 ...
						 echo Convert_time(substr("".$check_comment."",-30,11));    }}
															echo"</td>";
															echo "<td class=\"text-center\">";
                      ...
                      swal({
                     title: 'คุณแน่ใจไหม?',
                    text: 'ต้องการจะลบ  ".$result['user']."  ใช่หรือไม่',
                    type: 'warning',
	                showCloseButton: true,
	                showCancelButton: true,
