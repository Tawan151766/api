            ...
            <div class="col-lg-12">
                <div class="<?php print panel_modify();?>">
                    <div class="<?php print $panel_heading;?>">
                        <i class="fa fa-download"></i>&nbsp;<strong>HOTSPOT&nbsp;IMPORT&nbsp;USER</strong>
                    </div>
                            ...
                            <div class="row">
                                <div class="col-lg-7 col-md-9 ">
                                    <div class="form-group">
                                        <?php
                                              if($account!="read"){
										   echo "<button name=\"submit\" type=\"submit\"  value=\"submit\" class=\"btn btn-success\" ><i class=\"fa fa-check\"></i>&nbsp;บันทึก&nbsp;</button>&nbsp;&nbsp;&nbsp;";
								}else{
									echo "<a name=\"submit\" type=\"submit\"  value=\"submit\" class=\"btn btn-success\" ><i class=\"fa fa-check\"></i>&nbsp;บันทึก&nbsp;</a>&nbsp;&nbsp;&nbsp;";
								}
				                       ?>
                                            <button class="btn btn-danger" type="reset">&nbsp;&nbsp;&nbsp;<i class="fa fa-refresh"></i>&nbsp;รีเซ็ต&nbsp;&nbsp;&nbsp;</button>
                                    </div>
                                </div>
                            </div>
...
if($imageFileType != "csv" ) {
  echo "<script language='javascript'>swal('เกิดข้อผิดพลาด!','กรุณาเลือกไฟล์นามสกุล .csv','info').then(function () {
...
echo "<script language='javascript'>swal('เกิดข้อผิดพลาด!','ไม่มีข้อมูลในไฟล์','info').then(function () {
    window.location.href ='index.php?page=Import_Exel';}, function (dismiss) {
  if (dismiss === 'overlay') {
    window.location.href ='index.php?page=Import_Exel';
   }})</script>";
	...
	server_pro='".$updateDB[$i]['server']."',pass='".$updateDB[$i]['password']."', profile='".$updateDB[$i]['profile']."',limit_uptime='".$updateDB[$i]['limit-uptime']."',comment='".$updateDB[$i]['comment']."' WHERE user='".$updateDB[$i]['name']."'");}
	             echo "<script language='javascript'>swal('เกิดข้อผิดพลาด ".$project." Table!','database สำเร็จ ".($db_add+0)." และ hotspot สำเร็จ ".($mik_add+$mik_newadd)." กรุณาตรวจสอบ ".$check_group."','info').then(function () {
    window.location.href = 'index.php?page=listuser';}, function (dismiss) {
  if (dismiss === 'overlay') {
    window.location.href = 'index.php?page=listuser';
   }})</script>";
		exit();}else{echo "<script language='javascript'>swal('บันทึกสำเร็จแล้ว ".$project." User!','เพิ่ม  Group ".$db_new_group." สำเร็จแล้ว จำนวนทั้งหมด ".$mik_newadd." users','success').then(function () {
    window.location.href ='index.php?page=listuser';}, function (dismiss) {
  if (dismiss === 'overlay') {
    window.location.href ='index.php?page=listuser';
   }})</script>";
