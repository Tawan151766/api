<?php
			...
			if($total<"0"){
				echo "<script language='javascript'>swal('คุณไม่ได้เลือก!','กรุณาเลือกรายการ','error').then(function () {
...
$db_limit_uptime=$ARRAY2[$us]['limit-uptime'];if($limit_uptime==1){$db_limit_uptime="";}
	...
	mysql_query("UPDATE mt_gen SET profile='".$pro_set."',comment='".$com_set."',limit_uptime='".$db_limit_uptime."' WHERE user='".$user."'");	
			...
			echo "<script language='javascript'>swal('บันทึกสำเร็จ!','แก้ไข user ".$num1." จำนวนสำเร็จแล้ว','success').then(function () {
