<?php
	...
	if($username != ""){
		...
		if($rows>0){
			echo "<script language='javascript'>swal('เกิดข้อผิดพลาด!','มีชื่อ ".$username." แล้วในฐานข้อมูลกรุณาตั้งชื่อใหม่','error').then(function () {
  ...
  if (dismiss === 'overlay') {
    window.history.back();
		...
		echo "<script language='javascript'>swal('บันทึกสำเร็จแล้ว!','เพิ่ม ".$username." สำเร็จแล้ว','success').then(function () {
