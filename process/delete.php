<?php
	...
	$ARRAY = $API->comm("/ip/hotspot/user/remove", array(
		...
		echo "<script language='javascript'>swal('ลบเรียบร้อยแล้ว!','ลบ ".$user." สำเร็จแล้ว','success').then(function () {
  ...
  if (dismiss === 'overlay') {
	...
	if($_GET['return']=="userman"){
		...
		if($user==$username){echo "<script language='javascript'>swal('ไม่สามารถลบ User  ".$user."!','จะมีผลกับการสร้าง User ใน Profile ".$user."','error').then(function () {
  ...
  if (dismiss === 'overlay') {
	...
	if($_GET['return']=="mik"){
		...
		echo "<script language='javascript'>swal('ไม่สามารถลบ User  ".$user."!','ระบบไม่อนุญาตสิทธิ์ในการลบ','error').then(function () {
    window.location.href = 'index.php?page=mikrotikuser';}, function (dismiss) {
  if (dismiss === 'overlay') {
    window.location.href = 'index.php?page=mikrotikuser';}})</script>";
		exit;}
  ...
  if (dismiss === 'overlay') {
	...
	if($_GET['return']=="DB"){
     ...
     echo "<script language='javascript'>swal('ไม่สามารถลบ User  ".$user."!','ระบบไม่อนุญาตสิทธิ์ ในการลบ','error').then(function () {
      window.location.href = 'index.php?page=user&id=".$_GET['code']."';}, function (dismiss) {
      if (dismiss === 'overlay') {
