<?php
	...
	if($name != ""){
		...
		if($error>0){
			...
			echo "<script language='javascript'>swal('เกิดข้อผิดพลาด!','มีชื่อ ".$name." แล้วในฐานข้อมูลกรุณาตั้งชื่อใหม่','error').then(function () {
  ...
  if (dismiss === 'overlay') {
    window.history.back();
			...
			mysql_query("INSERT INTO mt_profile VALUE('".$name."','".$db_session."','".$idle."','".$keep."','".$auto."','".$total_validity."','".$shared."','".$limit."','".$price."','','','','','','','','','".$id."')");

			mysql_query("INSERT INTO mt_gen VALUE('".$name."','".$password."','','".$name."','','','','','','".$csv."','','".$group."','','".$date."','".$id."')");

			echo "<script language='javascript'>swal('บันทึกสำเร็จแล้ว!','เพิ่มโปรไฟล์ ".$name." สำเร็จแล้ว','success').then(function () {
