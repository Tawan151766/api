<?php
  ...
  $settotal_remove="".$scr_remno1."".$scr_remno2."".$scr_remno3."".$scr_remno4."".$scr_remno5."".$scr_remno6."".$scr_remno7."".$scr_remno8."".$scr_remno9."".$scr_remno10."".$end10."".$end9."".$end8."".$end7."".$end6."".$end5."".$end4."".$end3."".$end2."";
	...
	$start_time_disable="04:00:15";
	$start_time_remove="05:00:15";
	if($_REQUEST['show_error']==1){
		$show_error_expire=" else={:local nameERR [/ip hotspot user get \$i name];:log error \"ERROR HOTSPOT EXPIRE SCRIPT user :\$nameERR profile \$prof error by comment \$date\";}";
		$show_error_disable=" else={:local nameERR [/ip hotspot user get \$i name];:log error \"ERROR HOTSPOT DISABLE SCRIPT user :\$nameERR profile \$prof error by comment \$date\";}";
		$show_error_remove=" else={:local nameERR [/ip hotspot user get \$i name];:log error \"ERROR HOTSPOT REMOVE  SCRIPT user :\$nameERR profile \$prof error by comment \$date\";}";}else{
		$show_error_expire="";
		$show_error_disable="";
		$show_error_remove="";}

    $on_event_expire="{:global offset;:global today;:global prof;{:local date [ /system clock get date ];:local time [/system clock get time ];:local montharray ( \"jan\",\"feb\",\"mar\",\"apr\",\"may\",\"jun\",\"jul\",\"aug\",\"sep\",\"oct\",\"nov\",\"dec\" );:local year [ :pick \$date 7 11 ];:local monthdays;:if ((\$year % 4) = 0 ) do={:set monthdays ( 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );} else={:set monthdays ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );};:local days [ :pick \$date 4 6 ];:local daysmonth [ :pick \$date 0 3 ];:local monthtxt [ :pick \$date 0 3 ];:local months ([ :find \$montharray \$monthtxt]);:local hours [:pick \$time 0 2];:local minutes [:pick \$time 3 5];:local seconds [:pick \$time 6 8];:local todaytime ((\$hours*3600) + (\$minutes*60) + (\$seconds));:for nodays from=0 to=[:tonum \$months] do={:set days ( \$days + [ :pick \$monthdays \$nodays ]);:set daysmonth ( [ :pick \$monthdays \$nodays ] );};:set days ((\$year-2017) * 365  + \$days -\$daysmonth );:set today (( \$days * 86400 )+\$todaytime);};:foreach i in [ /ip hotspot user find where limit-uptime!=00:00:01 disabled=no(".$settotal_pro." )] do={:if ([ :find [ /ip hotspot user get \$i comment ] ] = 0 ) do={:local date [ /ip hotspot user get \$i comment ];:local montharray ( \"jan\",\"feb\",\"mar\",\"apr\",\"may\",\"jun\",\"jul\",\"aug\",\"sep\",\"oct\",\"nov\",\"dec\" );:local monthtxt [ :pick \$date 0 3 ];:local aaa ([ :find \$montharray \$monthtxt]);:if (\$aaa > -1) do={:set \$aaa 1;} else={:set \$aaa 0;};:local bbb [ :pick \$date 3 4 ];:local ccc [ :pick \$date 6 7 ];:local ddd (\"\",\"/\");:local eee ([ :find \$ddd \$bbb]);:local eee2 ([ :find \$ddd \$ccc]);:local fff [ :pick \$date 14 15 ];:local ggg [ :pick \$date 17 18 ];:local hhh (\"\",\":\");:local kkk ([ :find \$hhh \$fff]);:local kkk2 ([ :find \$hhh \$ggg]);:local check_comm (\$aaa+\$eee+\$eee2+\$kkk+\$kkk2);:if ((\$check_comm)=5) do={:local year [ :pick \$date 7 11 ];:local monthdays;:if ((\$year % 4) = 0 ) do={:set monthdays ( 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );} else={:set monthdays ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );};:local days [ :pick \$date 4 6 ];:local daysmonth [ :pick \$date 0 3 ];:local hours [:pick \$date 12 14];:local minutes [:pick \$date 15 17];:local seconds [:pick \$date 18 20];:local months ( [ :find \$montharray \$monthtxt ] );:local starttime ((\$hours*3600) + (\$minutes*60) + (\$seconds));:for nodays from=0 to=[:tonum \$months] do={:set days ( \$days + [ :pick \$monthdays \$nodays ] );:set daysmonth ( [ :pick \$monthdays \$nodays ] );:set prof [/ip hotspot user get \$i profile ];".$settotal_expire."};:set days ((\$year-2017) * 365  + \$days -\$daysmonth );:set days ( \$days * 86400 );:local conoffset ( \$offset * 3600 );:local stoplogoff (\$days + \$conoffset+\$starttime);:if ( \$stoplogoff < \$today ) do={ :local nameEXP [/ip hotspot user get \$i name];:log warning \"HOTSPOT EXPIRE SCRIPT: Profile \$prof Set expire user :\$nameEXP  first logged in \$date\";[ /ip hotspot user set \$i limit-uptime=1];[ /ip hotspot active remove [find where user=\$nameEXP] ];}}".$show_error_expire."}}}";

	$on_event_disable="{:global offset;:global today;:global prof;{:local date [ /system clock get date ];:local montharray ( \"jan\",\"feb\",\"mar\",\"apr\",\"may\",\"jun\",\"jul\",\"aug\",\"sep\",\"oct\",\"nov\",\"dec\" );:local year [ :pick \$date 7 11 ];:local monthdays;:if ((\$year % 4) = 0 ) do={:set monthdays ( 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );} else={:set monthdays ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );};:local days [ :pick \$date 4 6 ];:local daysmonth [:pick \$date 0 3 ];:local monthtxt [ :pick \$date 0 3 ];:local months ([:find \$montharray \$monthtxt]);:for nodays from=0 to=[:tonum \$months] do={:set days ( \$days + [:pick \$monthdays \$nodays ]);:set daysmonth ([:pick \$monthdays \$nodays ]);};:set days ((\$year-2017) * 365  + \$days -\$daysmonth );:set today ( \$days * 24 );};:foreach i in [ /ip hotspot user find where limit-uptime=00:00:01 disabled=no(".$settotal_pro." )] do={:if ([ :find [ /ip hotspot user get \$i comment ] ] = 0 ) do={:local date [ /ip hotspot user get \$i comment ];:local montharray ( \"jan\",\"feb\",\"mar\",\"apr\",\"may\",\"jun\",\"jul\",\"aug\",\"sep\",\"oct\",\"nov\",\"dec\" );:local monthtxt [ :pick \$date 0 3 ];:local aaa ([ :find \$montharray \$monthtxt]);:if (\$aaa > -1) do={:set \$aaa 1;}:local bbb [ :pick \$date 3 4 ];:local ccc [ :pick \$date 6 7 ];:local ddd (\"\",\"/\");:local eee ([ :find \$ddd \$bbb]);:local eee2 ([ :find \$ddd \$ccc]);:local fff [ :pick \$date 14 15 ];:local ggg [ :pick \$date 17 18 ];:local hhh (\"\",\":\");:local kkk ([ :find \$hhh \$fff]);:local kkk2 ([ :find \$hhh \$ggg]);:local check_comm (\$aaa+\$eee+\$eee2+\$kkk+\$kkk2);:if ((\$check_comm)=5) do={:local year [ :pick \$date 7 11 ];:local monthdays;:if ((\$year % 4) = 0 ) do={:set monthdays ( 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );} else={:set monthdays ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );};:local days [ :pick \$date 4 6 ];:local daysmonth [ :pick \$date 0 3 ];:local months ( [ :find \$montharray \$monthtxt ] );:for nodays from=0 to=[:tonum \$months] do={:set days ( \$days + [ :pick \$monthdays \$nodays ] );:set daysmonth ( [ :pick \$monthdays \$nodays ] );:set prof [/ip hotspot user get \$i profile ];".$settotal_expire."};:set days ((\$year-2017) * 365  + \$days -\$daysmonth );:if ( ((\$days*24) + \$offset) < \$today ) do={ :local nameDIS [/ip hotspot user get \$i name];:log warning \"HOTSPOT DISABLE EXPIRE USERS SCRIPT:Disabling Hotspot user \$nameDIS first logged in \$date\"; [ /ip hotspot user disable \$i ];}}".$show_error_disable."}}}";

	$on_event_remove="{:global offset;:global today;:global prof;{:local date [ /system clock get date ];:local montharray ( \"jan\",\"feb\",\"mar\",\"apr\",\"may\",\"jun\",\"jul\",\"aug\",\"sep\",\"oct\",\"nov\",\"dec\" );:local year [ :pick \$date 7 11 ];:local monthdays;:if ((\$year % 4) = 0 ) do={:set monthdays ( 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );} else={:set monthdays ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );};:local days [ :pick \$date 4 6 ];:local daysmonth [:pick \$date 0 3 ];:local monthtxt [ :pick \$date 0 3 ];:local months ([:find \$montharray \$monthtxt]);:for nodays from=0 to=[:tonum \$months] do={:set days ( \$days + [:pick \$monthdays \$nodays ]);:set daysmonth ([:pick \$monthdays \$nodays ]);};:set days ((\$year-2017) * 365  + \$days -\$daysmonth );:set today ( \$days * 24 );};:foreach i in [ /ip hotspot user find where limit-uptime=00:00:01 disabled=yes(".$settotal_pro." )] do={:if ([ :find [ /ip hotspot user get \$i comment ] ] = 0 ) do={:local date [ /ip hotspot user get \$i comment ];:local montharray ( \"jan\",\"feb\",\"mar\",\"apr\",\"may\",\"jun\",\"jul\",\"aug\",\"sep\",\"oct\",\"nov\",\"dec\" );:local monthtxt [ :pick \$date 0 3 ];:local aaa ([ :find \$montharray \$monthtxt]);:if (\$aaa > -1) do={:set \$aaa 1;}:local bbb [ :pick \$date 3 4 ];:local ccc [ :pick \$date 6 7 ];:local ddd (\"\",\"/\");:local eee ([ :find \$ddd \$bbb]);:local eee2 ([ :find \$ddd \$ccc]);:local fff [ :pick \$date 14 15 ];:local ggg [ :pick \$date 17 18 ];:local hhh (\"\",\":\");:local kkk ([ :find \$hhh \$fff]);:local kkk2 ([ :find \$hhh \$ggg]);:local check_comm (\$aaa+\$eee+\$eee2+\$kkk+\$kkk2);:if ((\$check_comm)=5) do={:local year [ :pick \$date 7 11 ];:local monthdays;:if ((\$year % 4) = 0 ) do={:set monthdays ( 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );} else={:set monthdays ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );};:local days [ :pick \$date 4 6 ];:local daysmonth [ :pick \$date 0 3 ];:local months ( [ :find \$montharray \$monthtxt ] );:for nodays from=0 to=[:tonum \$months] do={:set days ( \$days + [ :pick \$monthdays \$nodays ] );:set daysmonth ( [ :pick \$monthdays \$nodays ] );:set prof [/ip hotspot user get \$i profile ];".$settotal_remove."};:set days ((\$year-2017) * 365  + \$days -\$daysmonth );:if ( ((\$days*24) + \$offset) < \$today ) do={ :local nameREM [/ip hotspot user get \$i name];:log warning \"HOTSPOT REMOVE  USER DISABLED SCRIPT:remove Hotspot user \$nameREM first logged in \$date\"; [ /ip hotspot user remove \$i ];}}".$show_error_remove."}}}";
