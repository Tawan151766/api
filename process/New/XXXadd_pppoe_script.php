<?php
  ...
  $settotal_remove="".$scr_remno1."".$scr_remno2."".$scr_remno3."".$scr_remno4."".$scr_remno5."".$scr_remno6."".$scr_remno7."".$scr_remno8."".$scr_remno9."".$scr_remno10."".$end10."".$end9."".$end8."".$end7."".$end6."".$end5."".$end4."".$end3."".$end2."";
	...
	$show_error_expire=" else={:local PnameERR [/ppp secret get \$ii name]; :log error \"ERROR PPPOE EXPIRE SCRIPT user :\$PnameERR profile \$PPPprof error by comment \$PPPdate\";}";
	$show_error_remove=" else={:local PnameERR [/ppp secret get \$ii name]; :log error \"ERROR PPPOE REMOVE SCRIPT user :\$PnameERR profile \$PPPprof error by comment \$PPPdate\";}";}else{$show_error_expire="";$show_error_remove="";}

    $on_event_expire="{:global PPPoffset;:global PPPtoday;:global PPPprof;{:local PPPdate [ /system clock get date ];:local PPPtime [/system clock get time ];:local montharray ( \"jan\",\"feb\",\"mar\",\"apr\",\"may\",\"jun\",\"jul\",\"aug\",\"sep\",\"oct\",\"nov\",\"dec\" );:local year [ :pick \$PPPdate 7 11 ];:local monthdays;:if ((\$year % 4) = 0 ) do={:set monthdays ( 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );} else={:set monthdays ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );};:local days [ :pick \$PPPdate 4 6 ];:local daysmonth [ :pick \$PPPdate 0 3 ];:local monthtxt [ :pick \$PPPdate 0 3 ];:local months ([ :find \$montharray \$monthtxt]);:local hours [:pick \$PPPtime 0 2];:local minutes [:pick \$PPPtime 3 5];:local seconds [:pick \$PPPtime 6 8];:local PPPtodaytime ((\$hours*3600) + (\$minutes*60) + (\$seconds));:for nodays from=0 to=[:tonum \$months] do={:set days ( \$days + [ :pick \$monthdays \$nodays ]);:set daysmonth ( [ :pick \$monthdays \$nodays ] );};:set days ((\$year-2017) * 365  + \$days -\$daysmonth );:set PPPtoday (( \$days * 86400 )+\$PPPtodaytime);};:foreach ii in [ /ppp secret find where disabled=no(".$settotal_pro." )] do={:if ([ :find [ /ppp secret get \$ii comment ] ] = 0 ) do={:local PPPdate [ /ppp secret get \$ii comment ];:local montharray ( \"jan\",\"feb\",\"mar\",\"apr\",\"may\",\"jun\",\"jul\",\"aug\",\"sep\",\"oct\",\"nov\",\"dec\" );:local year [ :pick \$PPPdate 7 11 ];:local monthdays;:if ((\$year % 4) = 0 ) do={:set monthdays ( 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );} else={:set monthdays ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );};:local days [ :pick \$PPPdate 4 6 ];:local daysmonth [ :pick \$PPPdate 0 3 ];:local monthtxt [ :pick \$PPPdate 0 3 ];:local Paaa ([ :find \$montharray \$monthtxt]);:if (\$Paaa > -1) do={:set \$Paaa 1;} else={:set \$Paaa 0;};:local Pbbb [ :pick \$PPPdate 3 4 ];:local Pccc [ :pick \$PPPdate 6 7 ];:local Pddd (\"/\");:local Peee ([ :find \$Pddd \$Pbbb \$Pccc]);:if (\$Peee=0) do={:set \$Peee 1;} else={:set \$Peee 0;};:local Pfff [ :pick \$PPPdate 14 15 ];:local Pggg [ :pick \$PPPdate 17 18 ];:local Phhh (\":\");:local Pkkk ([ :find \$Phhh \$Pfff \$Pggg]);:if (\$Pkkk=0) do={:set \$Pkkk 1;} else={:set \$Pkkk 0;};:if ((\$Paaa+\$Peee+\$Pkkk)=3) do={:local hours [:pick \$PPPdate 12 14];:local minutes [:pick \$PPPdate 15 17];:local seconds [:pick \$PPPdate 18 20];:local months ( [ :find \$montharray \$monthtxt ] );:local starttime ((\$hours*3600) + (\$minutes*60) + (\$seconds));:for nodays from=0 to=[:tonum \$months] do={:set days ( \$days + [ :pick \$monthdays \$nodays ] );:set daysmonth ( [ :pick \$monthdays \$nodays ] );:set PPPprof [/ppp secret get \$ii profile ];".$settotal_expire."};:set days ((\$year-2017) * 365  + \$days -\$daysmonth );:set days ( \$days * 86400 );:local conPPPoffset ( \$PPPoffset * 3600 );:local stoplogoff (\$days + \$conPPPoffset+\$starttime);:if ( \$stoplogoff < \$PPPtoday ) do={ :local pppEXP [/ppp secret get \$ii name];:log warning \"PPPOE EXPIRE SCRIPT: Profile \$PPPprof disable user :\$pppEXP  first logged in \$PPPdate\";[ /ppp secret disable \$ii];[ /ppp active remove [find where name=\$pppEXP] ];}}".$show_error_expire."}}}";

	

	$on_event_remove="{:global PPPoffset;:global PPPtoday;:global PPPprof;{:local PPPdate [ /system clock get date ];:local montharray ( \"jan\",\"feb\",\"mar\",\"apr\",\"may\",\"jun\",\"jul\",\"aug\",\"sep\",\"oct\",\"nov\",\"dec\" );:local year [ :pick \$PPPdate 7 11 ];:local monthdays;:if ((\$year % 4) = 0 ) do={:set monthdays ( 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );} else={:set monthdays ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );};:local days [ :pick \$PPPdate 4 6 ];:local daysmonth [:pick \$PPPdate 0 3 ];:local monthtxt [ :pick \$PPPdate 0 3 ];:local months ([:find \$montharray \$monthtxt]);:for nodays from=0 to=[:tonum \$months] do={:set days ( \$days + [:pick \$monthdays \$nodays ]);:set daysmonth ([:pick \$monthdays \$nodays ]);};:set days ((\$year-2017) * 365  + \$days -\$daysmonth );:set PPPtoday ( \$days * 24 );};:foreach ii in [ /ppp secret find where disabled=yes(".$settotal_pro." )] do={:if ([ :find [ /ppp secret get \$ii comment ] ] = 0 ) do={:local PPPdate [ /ppp secret get \$ii comment ];:local montharray ( \"jan\",\"feb\",\"mar\",\"apr\",\"may\",\"jun\",\"jul\",\"aug\",\"sep\",\"oct\",\"nov\",\"dec\" );:local year [ :pick \$PPPdate 7 11 ];:local monthdays;:if ((\$year % 4) = 0 ) do={:set monthdays ( 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );} else={:set monthdays ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );};:local days [ :pick \$PPPdate 4 6 ];:local daysmonth [ :pick \$PPPdate 0 3 ];:local monthtxt [ :pick \$PPPdate 0 3 ];:local Paaa ([ :find \$montharray \$monthtxt]);:if (\$Paaa > -1) do={:set \$Paaa 1;} else={:set \$Paaa 0;};:local Pbbb [ :pick \$PPPdate 3 4 ];:local Pccc [ :pick \$PPPdate 6 7 ];:local Pddd (\"/\");:local Peee ([ :find \$Pddd \$Pbbb \$Pccc]);:if (\$Peee=0) do={:set \$Peee 1;} else={:set \$Peee 0;};:local Pfff [ :pick \$PPPdate 14 15 ];:local Pggg [ :pick \$PPPdate 17 18 ];:local Phhh (\":\");:local Pkkk ([ :find \$Phhh \$Pfff \$Pggg]);:if (\$Pkkk=0) do={:set \$Pkkk 1;} else={:set \$Pkkk 0;};:if ((\$Paaa+\$Peee+\$Pkkk)=3) do={:local months ( [ :find \$montharray \$monthtxt ] );:for nodays from=0 to=[:tonum \$months] do={:set days ( \$days + [ :pick \$monthdays \$nodays ] );:set daysmonth ( [ :pick \$monthdays \$nodays ] );:set PPPprof [/ppp secret get \$ii profile ];".$settotal_remove."};:set days ((\$year-2017) * 365  + \$days -\$daysmonth );:if ( ((\$days*24) + \$PPPoffset) < \$PPPtoday ) do={ :local pppREM [/ppp secret get \$ii name];:log warning \"PPPOE REMOVE  USER DISABLED SCRIPT:remove pppoe user \$pppREM first logged in \$PPPdate\"; [ /ppp secret remove \$ii ];}}".$show_error_remove."}}}";
