<?php
	...
	if($name != ""){
		...
		if($rows>0){
			...
			echo "<script language='javascript'>swal('เกิดข้อผิดพลาด!','มีชื่อ ".$name." แล้วในฐานขื้อมูลกรุณาตั้งชื่อใหม่','error').then(function () {
    window.history.back();}, function (dismiss) {
  if (dismiss === 'overlay') {
    window.history.back();
   }})</script>";
		}else{
			mysql_query("INSERT INTO pppoe_pro VALUE('".$name."','".$local."','".$remote."','".$session."','".$limit."','','".$price."','','','','','','','','','".$id."')");
