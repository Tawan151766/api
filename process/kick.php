	                    ...
	                    if ($_GET['return']=="mik"){
	                        ...
	                        $no=($no+1);
							  ...
							  $ARRAY4 = $API->comm("/ip/hotspot/active/remove
                             	...
                             	echo "<script language='javascript'>swal('เตะสำเร็จแล้ว!','เตะ User ".$_GET['user']." จำนวน ".$no." สำเร็จแล้ว','success').then(function () {
						...
						=.id=".$user."");
	                        ...
	                        echo "<script language='javascript'>swal('เตะสำเร็จแล้ว!','เตะ User ".$_GET['user']." จำนวน ".$no." สำเร็จแล้ว','success').then(function () {
								...
								if ($_GET['return']=="host"){
								 ...
								 echo "<script language='javascript'>swal('เตะ!','Kick IP. ".$_GET['user']." จำนวน ".$no." สำเร็จแล้ว','success').then(function () {
						       ...
						       =.id=".$user."");
	                           	...
	                           	echo "<script language='javascript'>swal('เตะสำเร็จแล้ว!','เตะ User ".$_GET['user']." สำเร็จแล้ว','success').then(function () {
                              window.history.back();}, function (dismiss) {
                                if (dismiss === 'overlay') {
                              window.history.back();}})</script>";
	                           exit;
								...
								echo "<script language='javascript'>swal('เกิดข้อผิดพลาด','กรุณาลองใหม่','error').then(function () {
