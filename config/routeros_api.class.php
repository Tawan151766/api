...
class routeros_api
{
     ...
     * @param string      $port_api   The RouterOS port api service
     * @return boolean                If we are connected or not
     */
    function connect($ip, $login, $password, $port_api)
    {
        for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
            $this->connected = false;
            $this->debug('Connection attempt #' . $ATTEMPT . ' to ' . $ip . ':' . $port_api . '...');
            $this->socket = @fsockopen($ip, $port_api, $this->error_no, $this->error_str, $this->timeout);
            if ($this->socket) {
